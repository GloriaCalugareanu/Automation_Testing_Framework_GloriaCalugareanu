...
public class GeneralSteps {
    ...
    public void isOpened(String pageName) {
        Page.navigateToPage(pageName, context.getWebDriverManager().getActiveWebDriver());
    }

    @Then("^Correct \"([^\"]*)\" endpoint is displayed$")
    public void correctEndpointIsDisplayed(String pageName) {
        String expectedUrlEndpoint = Page.pageContainsCorrectEndpoint(pageName, context.getWebDriverManager().getActiveWebDriver());
        String valoareaActualaEndpoint = context.getWebDriverManager().getActiveWebDriver().getCurrentUrl();
